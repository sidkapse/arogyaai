  ...
  async function send() {
    ...
    try {
      ...
      const msg = e instanceof MissingKeyError
        ? t('chat.missingKey')
        : e instanceof Error ? e.message : t('chat.errorRetry');
      setError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm">
            {ailment ? t('chat.askAbout', { title: ailment.title }) : t('chat.askAnything')}
          </span>
        </div>
        {onClose && (
        ...
        {history.length === 0 && (
          <div className="flex flex-col items-center gap-3 pt-10 text-center text-slate-400">
            ...
            <p className="text-sm max-w-xs">
              {ailment
                ? t('chat.emptyHintAilment', { title: ailment.title })
                : t('chat.emptyHintGlobal')}
            </p>
          </div>
        )}
